    ...
    REAL :: iTimes1, iTimes2, iTimes3, iTimes4
    INTEGER :: zetaStep=1
    ...
    Open (23, File='timeLog.dat', Status='unknown')
...
!        Write(6,*)'calculate Jacobi matrix ...'
 ...
 CALL CPU_TIME(iTimes2)
    elapsed_time = iTimes2 - iTimes1
    WRITE(*,*) 'Run CPU time:', elapsed_time, 'seconds'

    Write(7, 100)rerror

    CALL CPU_TIME(iter_finish)
    elapsed_time = iter_finish - iter_start
    WRITE(23,*) 'Iter', counter ,' CPU time:', elapsed_time, 'seconds'
    WRITE(*,*) 'Iter', counter ,' CPU time:', elapsed_time, 'seconds'

    If(rerror>epsi .and. counter<20) Goto 101

    Write(17, *)1.0

    CALL CPU_TIME(cpu_finish)
    elapsed_time = cpu_finish - cpu_start
    WRITE(23,*) 'Elapsed CPU time:', elapsed_time, 'seconds'
...
360 ks = ks - 1
    ...
    ! WRITE(*,*) 'one FP CPU time:', elapsed_time, 'seconds'
